...
"""
Post-processor for descriptor_ibi state transition logs.

Reads descriptor_ibi_fsm_transitions.log produced by the bind-module tracker
...
Usage:
    python parse_descriptor_ibi_fsm_log.py [descriptor_ibi_fsm_transitions.log]
"""
...
# Numeric ID -> name mapping from state_e (descriptor_ibi.sv)
STATE_NAMES = {
    0: "Idle",
    1: "DescLatch",
    2: "DescPop",
...
def parse_log(path):
    """Parse descriptor_ibi_fsm_transitions.log, return list of (timestamp_str, old_state, new_state)."""
    transitions = []
    with open(path) as f:
...
def print_readable_log(transitions):
    ...
    print("=" * 60)
    print("DESCRIPTOR_IBI FSM STATE TRANSITION LOG")
    print("=" * 60)
    print(f"{'Timestamp':>14s}  {'Old State':<12s} -> {'New State':<12s}")
    print("-" * 60)
    for ts_raw, old_st, new_st in transitions:
        print(f"{ts_raw:>14s}  {old_st:<12s} -> {new_st:<12s}")
...
def print_frequency_matrix(transitions):
    ...
    for (old_st, new_st), count in sorted(freq.items(), key=lambda x: -x[1]):
        print(f"  {old_st:<12s} -> {new_st:<12s}  : {count:>6d}")
...
def print_time_in_state(transitions):
    ...
    print("=" * 60)
    print(f"  {'State':<12s} {'Count':>6s} {'Min':>12s} {'Max':>12s} {'Total':>12s}")
    print("  " + "-" * 52)
    for state in sorted(state_durations.keys()):
        durs = state_durations[state]
        print(f"  {state:<12s} {len(durs):>6d} {min(durs):>12.1f} {max(durs):>12.1f} {sum(durs):>12.1f}")
...
def print_unexpected_transitions(transitions):
    ...
    if not unexpected:
        ...
        for ts_raw, old_st, new_st in unexpected:
            print(f"    {ts_raw:>14s}  {old_st:<12s} -> {new_st:<12s}  *** REVIEW ***")


def main():
    parser = argparse.ArgumentParser(
        description="Parse and analyze descriptor_ibi state transition logs.")
    parser.add_argument("logfile", nargs="?", default="descriptor_ibi_fsm_transitions.log",
                        help="Path to descriptor_ibi_fsm_transitions.log (default: descriptor_ibi_fsm_transitions.log)")
