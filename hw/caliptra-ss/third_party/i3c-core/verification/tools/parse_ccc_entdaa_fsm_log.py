...
"""
Post-processor for ccc_entdaa state transition logs.

Reads ccc_entdaa_fsm_transitions.log produced by the bind-module tracker
...
Usage:
    python parse_ccc_entdaa_fsm_log.py [ccc_entdaa_fsm_transitions.log]
"""
...
# Numeric ID -> name mapping from state_e (ccc_entdaa.sv)
# Values are explicitly assigned in the RTL.
STATE_NAMES = {
    0x0: "Idle",
    0x1: "WaitStart",
    0x2: "ReceiveRsvdByte",
...
EXPECTED_TRANSITIONS = {
    ...
    ("Idle", "WaitStart"),
    # WaitStart
    ("WaitStart", "ReceiveRsvdByte"),
    ("WaitStart", "Done"),
    # ReceiveRsvdByte
    ("ReceiveRsvdByte", "AckRsvdByte"),
    ("ReceiveRsvdByte", "SendNack"),
    ("ReceiveRsvdByte", "Done"),
    # AckRsvdByte
    ("AckRsvdByte", "SendID"),
    ("AckRsvdByte", "Done"),
    # SendNack
    ...
    ("PrepareIDBit", "Done"),
    # SendIDBit
    ("SendIDBit", "LostArbitration"),
    ("SendIDBit", "ReceiveAddr"),
    ("SendIDBit", "PrepareIDBit"),
    ("SendIDBit", "Done"),
    # LostArbitration
    ("LostArbitration", "WaitStart"),
    ("LostArbitration", "Done"),
    # ReceiveAddr
    ("ReceiveAddr", "AckAddr"),
    ("ReceiveAddr", "SendNack"),
    ("ReceiveAddr", "Done"),
    # AckAddr
    ("AckAddr", "Done"),
    # Done
...
def parse_log(path):
    """Parse ccc_entdaa_fsm_transitions.log, return list of (timestamp_str, old_state, new_state)."""
    transitions = []
    with open(path) as f:
...
def print_readable_log(transitions):
    ...
    print("=" * 66)
    print("CCC_ENTDAA FSM STATE TRANSITION LOG")
    print("=" * 66)
...
def main():
    parser = argparse.ArgumentParser(
        description="Parse and analyze ccc_entdaa state transition logs.")
    parser.add_argument("logfile", nargs="?", default="ccc_entdaa_fsm_transitions.log",
                        help="Path to ccc_entdaa_fsm_transitions.log (default: ccc_entdaa_fsm_transitions.log)")
